...
- **Frontend (APPLIED AT SOURCE 2026-08-13)**: presentation surfaces letterbox in dark ink - build.py now stamps `data-mode` on the html element; base.css paints the body ink for `data-mode="presentation"` and `html:fullscreen`. A light frame around a scaled slide read as broken white borders on full-bleed photo slides.
