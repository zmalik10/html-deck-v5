...
- **House-wide (APPLIED AT SOURCE 2026-08-13, owner directive)**: removed all accent dashes/ticks - `.label::before` deleted from frontend/base.css, `rule()` in render_slides.py now a no-op. Remaining per-template decorative bars (quote underlines etc.) should be reviewed against the same directive at next gallery pass.
- **Frontend (APPLIED AT SOURCE 2026-08-13, v2 after owner correction)**: letterbox around the scaled stage is ALWAYS the theme deck background (seamless on text slides); full-bleed photo slides get a build-injected .slide-bleed layer - an ambient blurred copy of their own background image - so the photo reaches all four viewport corners. Screen-only (hidden in print/export). The first attempt (dark ink letterbox on all slides) was rejected: black borders on white slides are wrong.
- **Fonts (APPLIED AT SOURCE 2026-08-13)**: the deck embedded only Montserrat 400/700/900, so authored intermediate weights snapped HEAVIER by the browser's font-matching (600->700, 800->900) - everything read as bolded. Added montserrat-500/600/800.woff2 to assets/fonts and extended build.py font_css to embed all six weights. TTF folder for PPTX reviewers should gain Medium/SemiBold/ExtraBold at next pass.
- **cover_geo**: subhead hard-styled at 24px/700 - display-scale bold for supporting copy. Correct at renderer to the weight ladder (19px/500, ~640px measure) via the gate.
- **NM-19 (photo persona columns)**: captions beneath column images must be centered (owner directive 2026-08-13; banked as a house rule in SKILL.md).
- **NEW PATTERN to promote (owner-praised 2026-08-13)**: the "punch-list" concept slide from affiliate-partner-program s6 - full-bleed photo + ink scrim, big white statement left, white ruled checklist sheet right (check squares, navy row titles, body text). Candidate for a v9 template (e.g. NM-25 "Checklist Sheet Over Photo") via the gate; renderer source in that deck's patch_slides.py.
