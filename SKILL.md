...
- **Photo-column captions are CENTERED.** In persona/photo-column layouts (a column of image + title + one-liner), the title and description beneath each image are center-aligned under it - left-aligned captions read as misaligned against the centered image mass. (Icon CARDS keep centered icon+title with left-aligned description - that rule is for prose-weight card bodies, not column captions.)
